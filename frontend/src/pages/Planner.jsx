...
export default function Planner() {
  ...
  const calculateTaskDayName = (startDateStr, taskIndex) => {
    ...
    // Parse the ISO date string safely
  ...
  const processedTasks = plan?.tasks ? plan.tasks.map((task, index) => {
    return {
      ...task,
      displayDay: task.day || calculateTaskDayName(plan.start_date, index)
    };
  }) : [];
              ...
              <div className="flex gap-1.5 overflow-x-auto pb-2 border-b border-slate-900 no-scrollbar">
                {DAYS_OF_WEEK.map((day) => (
                  <button
                    ...
                    className={`py-1.5 px-3.5 rounded-xl text-xs font-semibold cursor-pointer shrink-0 transition-all ${
                      activeDayFilter === day
                    ...
                    return (
                      ...
                      >
                        <button
                          ...
                          className={`mt-0.5 shrink-0 inline-flex h-5 w-5 items-center justify-center rounded-md border transition cursor-pointer ${
                            isDone
