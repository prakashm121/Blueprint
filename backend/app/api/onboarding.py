...
def submit_goals(
    ...
    # New step order: goals -> assessment (Core Subjects + DSA) -> role_skills -> generate_roadmap
    current_user.onboarding_step = "assessment"
    db.commit()
    return StepResponse(onboarding_step="assessment")
...
async def generate_roadmap(
    ...
    # Seed all role-specific skills at confidence=25 (idempotent — skip if already rated)
    DEFAULT_SEED_CONFIDENCE = 25
        ...
        for s in skills:
            ...
            if not exists:
                db.add(UserSkillAssessment(
                    ...
                    role=target_role,
                    self_rated_confidence=DEFAULT_SEED_CONFIDENCE,
                ))
